...
function getGroundSprite(tx, ty) {
  const mx = mod(tx, 8), my = mod(ty, 8);
  const road = mx === 3 || mx === 4 || my === 3 || my === 4;
  const h = hash2(tx, ty);
  return road ? ((h & 1) === 0 ? 'roadA' : 'roadB') : ((h & 1) === 0 ? 'sidewalkA' : 'sidewalkB');
}
...
const enemies = [], projectiles = [], xpGems = [];
...
function itemSummary() {
  const entries = [...acquiredItems.entries()];
  if (!entries.length) return 'Items: none';
  return `Items: ${entries.slice(-4).map(([name, count]) => `${name}${count > 1 ? ` x${count}` : ''}`).join(' / ')}`;
}
...
function chooseUpgrade(choice) {
  choice.apply(); rememberItem(choice.name); message = `${choice.name}: ${choice.desc}`; messageTimer = 3; closeLevelUp();
  if (queuedLevelUps > 0) { queuedLevelUps -= 1; requestAnimationFrame(openLevelUp); }
}
function gainXp(amount) {
  player.xp += amount;
  while (player.xp >= player.nextXp) { player.xp -= player.nextXp; player.level += 1; player.nextXp = Math.floor(player.nextXp * 1.28 + 7); queuedLevelUps += 1; }
  if (queuedLevelUps > 0 && !levelUpOpen) { queuedLevelUps -= 1; openLevelUp(); }
}
...
function spawnEnemy() {
  ...
  enemies.push({
    ...
    r: def.r, hp: def.hp * waveScale, maxHp: def.hp * waveScale,
    speed: def.speed * (1 + elapsed / 420), damage: def.damage, xp: def.xp, sprite: type, dead: false,
  });
}
...
function dropXpGem(enemy) {
  ...
  xpGems.push({ x: enemy.x, y: enemy.y, value: enemy.xp, r: 7 });
}
...
function update(dt) {
  ...
  if (mx !== 0 || my !== 0) { player.x += movement.x * player.speed * dt; player.y += movement.y * player.speed * dt; player.facing = movement.x < 0 ? -1 : movement.x > 0 ? 1 : player.facing; }
  ...
  if (player.fireCooldown <= 0) { player.fireCooldown = player.fireRate; fireProjectile(); }
  for (const enemy of enemies) {
    if (enemy.dead) continue;
    const dir = normalize(player.x - enemy.x, player.y - enemy.y);
    enemy.x += dir.x * enemy.speed * dt; enemy.y += dir.y * enemy.speed * dt;
    if (overlaps(player.x, player.y, player.r, enemy.x, enemy.y, enemy.r) && player.invuln <= 0) {
      player.hp -= enemy.damage; player.invuln = 0.42;
      if (player.hp <= 0) { player.hp = 0; gameOver = true; message = 'Game Over - refresh to restart the penguin.'; messageTimer = 999; }
    }
  }
  rebuildEnemyGrid();
  ...
  for (let i = projectiles.length - 1; i >= 0; i -= 1) {
    ...
    if (!consumed) {
      ...
      if (enemy) {
        enemy.hp -= shot.damage; shot.hit.add(enemy.id);
        if (enemy.hp <= 0) { enemy.dead = true; killedEnemy = true; score += 1; dropXpGem(enemy); }
  ...
  for (let i = xpGems.length - 1; i >= 0; i -= 1) {
    ...
    if (d < player.pickupRange) {
      ...
      gem.x += dir.x * pull * 96 * dt; gem.y += dir.y * pull * 96 * dt;
    }
    if (d < player.r + gem.r) { gainXp(gem.value); xpGems.splice(i, 1); }
  }
}
...
function render() {
  const cameraX = Math.floor(player.x - view.width / 2), cameraY = Math.floor(player.y - view.height / 2);
  beginRender();
  const startTileX = Math.floor(cameraX / TILE) - 1, startTileY = Math.floor(cameraY / TILE) - 1;
  const endTileX = Math.ceil((cameraX + view.width) / TILE) + 1, endTileY = Math.ceil((cameraY + view.height) / TILE) + 1;
  for (let ty = startTileY; ty <= endTileY; ty += 1) {
    for (let tx = startTileX; tx <= endTileX; tx += 1) { pushQuad(getGroundSprite(tx, ty), tx * TILE, ty * TILE, TILE, TILE); drawCityDecals(tx, ty); }
  }
  ...
  const uiX = cameraX + 18, uiY = cameraY + 18;
...
function resize() {
  ...
  const width = Math.floor(view.width * dpr), height = Math.floor(view.height * dpr);
  if (canvas.width !== width || canvas.height !== height) { canvas.width = width; canvas.height = height; canvas.style.width = `${view.width}px`; canvas.style.height = `${view.height}px`; }
}
