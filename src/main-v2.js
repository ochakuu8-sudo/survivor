...
const PROJECTILE_QUERY_RADIUS = 44;
const MAX_UPGRADE_ITEMS = 24;
...
function drawZombieRunner(ctx) {
  ...
  rect(ctx, 9, 8, 2, 1, '#2a2225');
  rect(ctx, 11, 9, 2, 1, '#7c8f68');
...
function drawCrawler(ctx) {
  ...
  rect(ctx, 3, 12, 2, 1, '#7f1018');
  rect(ctx, 13, 11, 1, 1, '#81131a');
}
...
const spriteSources = {
  ...
  damageKit: makeSprite((ctx) => {
    rect(ctx, 3, 5, 10, 7, '#5d2024');
    rect(ctx, 4, 4, 8, 2, '#a82e37');
    rect(ctx, 5, 7, 6, 2, '#f04e56');
    rect(ctx, 7, 2, 2, 3, '#e6c17a');
    rect(ctx, 2, 8, 2, 2, '#2d1518');
    rect(ctx, 12, 8, 2, 2, '#2d1518');
  }),
  speedKit: makeSprite((ctx) => {
    rect(ctx, 3, 5, 10, 7, '#173a4a');
    rect(ctx, 4, 4, 8, 2, '#2f86a3');
    rect(ctx, 5, 7, 6, 2, '#7fe4ff');
    rect(ctx, 8, 2, 2, 3, '#c8f7ff');
    rect(ctx, 6, 10, 4, 1, '#0d1d25');
  }),
  bulletKit: makeSprite((ctx) => {
    rect(ctx, 3, 5, 10, 7, '#6b5120');
    rect(ctx, 4, 4, 8, 2, '#d19b2e');
    rect(ctx, 5, 7, 2, 3, '#ffdf6a');
    rect(ctx, 8, 7, 2, 3, '#ffdf6a');
    rect(ctx, 11, 7, 2, 3, '#ffdf6a');
    rect(ctx, 6, 2, 4, 2, '#fff0a8');
  }),
...
let gameOver = false;
let messageTimer = 4;
let message = 'Find weapon upgrades in the city: damage, fire rate, and extra bullets.';
...
const upgradeDefs = {
  damage: {
    ...
    apply() {
      ...
      player.projectileDamage += 4 + Math.floor(player.damageLevel / 4);
      message = `Damage Kit picked up: ATK ${player.projectileDamage}`;
    },
  },
  speed: {
    sprite: 'speedKit',
    ...
    apply() {
      player.speedLevel += 1;
      player.fireRate = Math.max(0.11, player.fireRate * 0.92);
      message = `Trigger Kit picked up: Fire ${player.fireRate.toFixed(2)}s`;
    },
  },
  bullets: {
    sprite: 'bulletKit',
    ...
    apply() {
      ...
      player.shotCount = Math.min(7, player.shotCount + 1);
      player.projectileSpeed += 12;
      message = `Ammo Rig picked up: Bullets x${player.shotCount}`;
    },
  },
};

function randomUpgradeType() {
  const roll = Math.random();
  if (roll < 0.38) return 'damage';
  if (roll < 0.70) return 'speed';
  return 'bullets';
}

function spawnUpgradeItem(forceNear = false) {
  const angle = rand(0, Math.PI * 2);
  const radius = forceNear ? rand(180, 420) : rand(360, 900);
  upgradeItems.push({
    x: clamp(player.x + Math.cos(angle) * radius, 48, WORLD_SIZE - 48),
    y: clamp(player.y + Math.sin(angle) * radius, 48, WORLD_SIZE - 48),
    type: randomUpgradeType(),
    r: 13,
  });
}

function ensureUpgradeItems(dt) {
  upgradeSpawnTimer -= dt;
  if (upgradeItems.length < 8) {
    spawnUpgradeItem(true);
    return;
  }
  if (upgradeSpawnTimer <= 0 && upgradeItems.length < MAX_UPGRADE_ITEMS) {
    upgradeSpawnTimer = rand(3.5, 7.5);
    spawnUpgradeItem(false);
  }
}
...
function fireProjectile() {
  ...
  const spread = count === 1 ? 0 : Math.min(0.72, 0.14 * (count - 1));
...
function update(dt) {
  ...
  ensureUpgradeItems(dt);

  for (let i = upgradeItems.length - 1; i >= 0; i -= 1) {
    const item = upgradeItems[i];
    if (overlaps(player.x, player.y - 10, player.r + 6, item.x, item.y, item.r)) {
      upgradeDefs[item.type].apply();
      messageTimer = 3.0;
...
function render() {
  ...
  for (const item of upgradeItems) {
    const def = upgradeDefs[item.type];
    pushQuad('shadow', item.x - 12, item.y - 4, 24, 10, [1, 1, 1, 0.32]);
    pushQuad(def.sprite, item.x - 12, item.y - 20, 24, 24, def.tint);
  }
  ...
  hud.innerHTML = `
    ...
    <div>HP ${Math.ceil(player.hp)}/${player.maxHp} | ATK ${player.projectileDamage} | Fire ${player.fireRate.toFixed(2)}s | Bullets x${player.shotCount}</div>
    <div>Kills ${score} | Zombies ${enemies.length}/${MAX_ENEMIES} | Upgrades ${upgradeItems.length} | Time ${Math.floor(elapsed)}s</div>
...
for (let i = 0; i < 12; i += 1) spawnUpgradeItem(true);
