...
import { beginBossOffer, collectWeaponDrops } from "./lootModel.js";
import { saveProgress } from "./progression.js";
import { enterUpgradeTree, appendBossChoices } from "./skillTree.js";
...
export function recordArenaKill(enemy) {
  ...
  if (e.phase === "horde" && !enemy.boss)
    e.kills = Math.min(difficulty(game.wave).kills, e.kills + 1);
}
  ...
  if (e.phase === "horde" && e.kills >= difficulty(game.wave).kills) {
    ...
    if (e.warning <= 0) {
      ...
      if (Math.hypot(delta.dx, delta.dy) < 150) {
        ...
        e.position = wrapDungeonPoint(game.dungeon, {
          x: game.player.x - Math.cos(angle) * 330,
          y: game.player.y - Math.sin(angle) * 330,
        });
        e.warning = 1;
        addTelegraphCircle(e.position.x, e.position.y, 88, 1);
        return;
      }
      const boss = spawnEnemy("bigZombie", {
        position: e.position,
        boss: true,
        noDeathChest: true,
      });
      ...
      boss.chargeWindup = 0.95;
      boss.chargeCooldown = Math.max(1.8, 3.2 - game.wave * 0.2);
    ...
    if (game.buildCombat)
      game.buildCombat.heal(Math.ceil(game.player.maxHp * 0.3), "bossHeal");
    else
      game.player.hp = Math.min(
        game.player.maxHp,
        game.player.hp + Math.ceil(game.player.maxHp * 0.3),
      );
    if (game.buildCombat) {
      game.buildCombat.objects = [];
      game.buildCombat.queue = [];
      game.buildCombat.visuals = [];
    }
    game.enemies = [];
    game.bullets = [];
...
export function advanceDifficulty() {
  if (
    game.mode !== "bossReward" ||
    game.encounter?.phase !== "reward" ||
    !game.loot.bossOffer?.selectedId ||
    game.loot.inbox.length
  )
    return false;
  game.loot.bossOffer = null;
  saveProgress(game);
...
export function renderBossReward() {
  ...
  kicker.textContent = ja
    ? "BOSS DEFEATED / 強化の時間"
    : "BOSS DEFEATED / TIME TO UPGRADE";
  const title = document.createElement("h1");
  title.textContent = ja
    ? `難易度 ${game.wave} 突破！`
    : `Difficulty ${game.wave} complete!`;
  const reward = document.createElement("p");
  reward.className = "boss-reward-gold";
  reward.textContent = `+${game.lastBossReward} G`;
  const info = document.createElement("p");
  info.textContent = ja
    ? `落ちているGを回収・HPを30%回復。所持 ${game.gold} G。`
    : `Loose gold collected. Restored 30% HP. Balance: ${game.gold} G.`;
  const next = document.createElement("p");
  next.textContent = ja
    ? `次は難易度 ${game.wave + 1}。${difficulty(game.wave + 1).kills}体倒すと次のボスが出現。`
    : `Next: difficulty ${game.wave + 1}. Defeat ${difficulty(game.wave + 1).kills} enemies to summon its boss.`;
  panel.append(kicker, title, reward, info, next);
  if (game.saveFailed) {
  ...
  skills.textContent = ja ? "武器庫で装備・リロール" : "Armory";
  skills.onclick = enterUpgradeTree;
  const go = document.createElement("button");
  go.className = "primary";
  go.textContent =
    game.wave === 5 ? "クリア結果へ" : ja ? "次の難易度へ" : "Next difficulty";
  go.disabled = !game.loot.bossOffer?.selectedId || !!game.loot.inbox.length;
  go.onclick = advanceDifficulty;
  if (game.wave === 5)
    next.textContent = "5体のボスを撃破！戦利品を選んでクリアしよう。";
  if (game.loot.inbox.length)
    info.textContent += ` 武器庫が満杯です。受取待ち ${game.loot.inbox.length}個を整理してください。`;
  const actions = document.createElement("div");
  actions.className = "boss-reward-actions";
  actions.append(skills, go);
