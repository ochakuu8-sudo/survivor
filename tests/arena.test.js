...
const { updateArenaProgress, advanceDifficulty } =
  await import("../src/arena.js");
const { difficulty } = await import("../src/difficulty.js");
const {
  spawnEnemy,
  spawnEnemies,
  enemyRunSpeedMultiplier,
  currentSpawnPlan,
  updateEnemies,
...
const { dropGold, collectAllGold, updateGoldDrops } =
  await import("../src/gold.js");
const { rewardForFloor, readProgress } = await import("../src/progression.js");
const {
  shortestDungeonDelta,
  wrapDungeonPoint,
  moveActorWithDungeonCollision,
} = await import("../src/dungeon.js");
const { enterUpgradeTree, continueFromSkillTree, lootAction } =
  await import("../src/skillTree.js");

const { claimBossWeapon, startReroll, chooseReroll, equipped } =
  await import("../src/lootModel.js");
const { createWeapon, xpNeeded, weaponSlots } =
  await import("../src/lootModel.js");
function claimBoss() {
  lootAction(() => claimBossWeapon(game, game.loot.bossOffer.choices[0].uid));
}
...
test("combat XP unlocks a slot and the frame persists that individual", () => {
  const w = equipped(game);
  w.xp = xpNeeded(1) + xpNeeded(2) + xpNeeded(3) - 1;
  game.enemies = [];
  killEnemy(
    spawnEnemy("walker", {
      position: { x: game.player.x + 100, y: game.player.y },
    }),
  );
  update(0);
  assert.equal(weaponSlots(w), 4);
  const stored = readProgress().loot.inventory.find(
    (other) => other.uid === w.uid,
  );
  assert.equal(stored.xp, w.xp);
  assert.equal(weaponSlots(stored), 4);
});

test("weapon pickup wraps across the map seam and distant loot survives death", () => {
  const d = game.dungeon,
    p = game.player;
  p.x = d.offsetX + d.width * 96 - 2;
  const near = createWeapon(game.loot, "B10"),
    far = createWeapon(game.loot, "G10");
  game.loot.worldDrops = [
    { x: d.offsetX + 2, y: p.y, weapon: near },
    { x: p.x - 900, y: p.y, weapon: far },
  ];
  update(1 / 60);
  assert.ok(game.loot.inventory.some((w) => w.uid === near.uid));
  assert.equal(game.loot.worldDrops.length, 1);
  finishRun("dead");
  assert.ok(readProgress().loot.inventory.some((w) => w.uid === far.uid));
  assert.equal(readProgress().loot.worldDrops.length, 0);
});

test("storage failure rolls back a paid reroll rather than losing gold", () => {
  game.loot.debugFree = false;
  game.gold = 45;
  const before = JSON.stringify(game.loot),
    write = globalThis.localStorage.setItem;
  try {
    globalThis.localStorage.setItem = () => {
      throw Error("quota");
    };
    assert.match(
      lootAction(() => startReroll(game, game.loot.equippedId, 0, () => 0)),
      /取り消しました/,
    );
    assert.equal(game.gold, 45);
    assert.equal(JSON.stringify(game.loot), before);
  } finally {
...
test("plain has no blockers or facilities and movement wraps continuously", () => {
  ...
  assert.equal(d.wrapEdges, true);
  assert.equal(d.exit, null);
  assert.deepEqual(d.obstacles, []);
...
test("time alone neither summons bosses nor increases enemy speed or spawn pressure", () => {
  const plan = currentSpawnPlan(),
    speed = enemyRunSpeedMultiplier();
  game.floorElapsed = 100000;
  updateArenaProgress(100000);
  assert.equal(game.encounter.phase, "horde");
  assert.equal(game.wave, 1);
  assert.deepEqual(currentSpawnPlan(), plan);
  assert.equal(enemyRunSpeedMultiplier(), speed);
...
test("one boss per threshold, warning moves away from player, boss kills do not prefill next tier", () => {
  ...
  for (let i = 0; i < 40; i++)
    killEnemy(spawnEnemy("walker", { position: marker }));
...
test("boss reward pays once, preserves position and map, pauses combat, and returns from skills", () => {
  ...
  killEnemy(boss);
  updateArenaProgress(0);
  assert.equal(game.mode, "bossReward");
  assert.equal(game.bossesDefeated, 1);
  assert.equal(game.gold, 30 + 45);
  assert.equal(game.player.hp, 10);
  assert.equal(
    game.enemies.length +
      game.bullets.length +
      game.enemyProjectiles.length +
      game.goldDrops.length,
    0,
  );
  const elapsed = game.runElapsed,
    gold = game.gold;
  update(10);
  updateArenaProgress(10);
  assert.equal(game.runElapsed, elapsed);
  assert.equal(game.gold, gold);
  enterUpgradeTree();
  assert.equal(game.mode, "upgradeTree");
  const uid = game.loot.equippedId;
  assert.equal(
    lootAction(() => startReroll(game, uid, 1, () => 0)),
    "",
  );
  assert.equal(
    lootAction(() => chooseReroll(game, 0)),
    "",
  );
  update(10);
  assert.equal(game.runElapsed, elapsed);
  ...
  assert.equal(game.dungeon, d);
  assert.deepEqual({ x: game.player.x, y: game.player.y }, pos);
  assert.ok(equipped(game).affixes[1]);
});
...
test("five boss victories clear the run and retain gold and skills on restart", () => {
  ...
  assert.equal(game.runResult.result, "clear");
  assert.equal(game.runResult.bosses, 5);
  assert.equal(game.runResult.difficulty, 5);
  ...
  resetRun();
  assert.equal(game.wave, 1);
  assert.equal(game.bossesDefeated, 0);
  assert.equal(game.gold, gold);
});
...
test("gold, friendly projectiles, hostile projectiles and separation work across map seams", () => {
  ...
  updateEnemyProjectiles(0.016);
  assert.equal(p.hp, 27);
  const a = spawnEnemy("walker", { position: { x: d.offsetX + 1, y: 300 } });
  const b = spawnEnemy("walker", {
    position: { x: d.offsetX + d.width * 96 - 1, y: 300 },
  });
  updateEnemies(0);
...
test("spawn caps are bounded and normal enemies spawn outside the actual desktop viewport", () => {
  ...
  for (const enemy of game.enemies) {
    const delta = shortestDungeonDelta(
      game.dungeon,
      game.camera.x,
      game.camera.y,
      enemy.x,
...
test("death keeps collected currency, resets encounter, and grants no fictional time bonus", () => {
  ...
  updateGoldDrops(0.01);
  finishRun("dead");
  assert.equal(game.runResult.survivalTime, 123);
  assert.equal(game.runResult.bonusPoints, 0);
  resetRun();
...
test("late boss volley is telegraphed and locks its aim before firing", () => {
  ...
  boss.bigZombieState = "chase";
  boss.chargeCooldownLeft = 99;
  boss.volleyTimer = 0;
  updateEnemies(0.016);
  assert.ok(boss.volleyWarning > 0);
  assert.equal(game.enemyProjectiles.length, 0);
  assert.equal(
    game.effects.filter((e) => e.type === "telegraphLine").length,
    5,
  );
  ...
  assert.ok(Math.abs(Math.atan2(middle.vy, middle.vx) - angle) < 0.00001);
});
